...
$BookMark = 'Overview'; // Anchor's id in the manual's html document.
...
echo '<p class="page_title_text">
		<img alt="" src="', $RootPath, '/css/', $_SESSION['Theme'], '/images/user.png" title="', _('Timesheets'), '" /> ', _('Timesheet Entry'), '
	</p>'; // Page title.
...
if (isset($_GET['SelectedEmployee'])) {
	if ($_GET['SelectedEmployee'] == 'NewSelection') {
		...
		$CheckUserResult = DB_query("SELECT employeeid FROM prlemployeemaster WHERE userid='" . $_SESSION['UserID'] . "'");
	...
	if (isset($SelectedEmployee)) { //Get the employee's details
		$SQL = "SELECT employeeid,
			...
			ON prlemployeemaster.stockid=stockmaster.stockid
			WHERE prlemployeemaster.employeeid='" . $SelectedEmployee . "'";
			...
			if (DB_num_rows($CheckTimesheetResult) == 1) {
				...
				$InsertTimsheetResult = DB_query("INSERT INTO timesheets (wo,
													...
													VALUES ('" . $_POST['WO'] . "',
															...
															'" . filter_number_format($_POST['Day7']) . "')", _('Could not add this timesheet record'));
				...
				while ($WeekTimeRow = DB_fetch_array($WeekTimeResult)) {
					...
					$SQL = "INSERT INTO stockmoves (stockid,
							...
							VALUES ('" . $WeekTimeRow['issueitem'] . "',
									...
									'" . $WeekTimeRow['labourcost'] . "',
									'0',
									'" . _('Timesheet for the week ending') . ' ' . $_POST['WeekEnding'] . ': ' . $WeekTimeRow['firstname'] . ' ' . $WeekTimeRow['lastname'] . "')";
					...
					$UpdateWOResult = DB_query("UPDATE workorders
											...
											WHERE wo='" . $WeekTimeRow['wo'] . "'", $ErrMsg, $DbgMsg, true);
				} //end loop through the WOs entered on this timesheet
				//change the status of the timesheet to approved
				$SubmittedTimesheetResult = DB_query("UPDATE timesheets
													SET status=2
												WHERE employeeid='" . $SelectedEmployee . "'
												AND weekending='" . FormatDateForSQL($_POST['WeekEnding']) . "'", _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' . _('Could not change the timesheet status to approved because'), _('The following SQL was used to update the work order'), true);
	...
	if (!isset($SelectedEmployee) and $_SESSION['CanEnterTimesheets'] == 1) {
		...
		$SQL = "SELECT employees.employeeid,
					...
					employees2.lastname as managersurname,
					employees.normalhours,
					employees.email1,
					employees.userid
			FROM prlemployeemaster AS employees LEFT JOIN prlemployeemaster AS employees2
			ON employees.manager=employees2.employeeid";
		...
		if (DB_num_rows($Result) > 0) {
			...
			<tr class="striped_row">
				...
				<th class="ascending">', _('Last Name'), '</th>
		...
		echo '<br />';
	} elseif ($_SESSION['CanEnterTimesheets'] == 1 and isset($SelectedEmployee)) {
	...
	if (isset($SelectedEmployee)) {
		...
		echo '<h2>', _('For'), ' ', $EmployeeRow['firstname'], ' ', $EmployeeRow['lastname'], ' ', _('For the week ending'), ': <select name="WeekEnding" onChange="ReloadForm(TimesheetForm.RefreshWeek)" >';
		...
		if (isset($_POST['WeekEnding'])) {
				...
				while ($TimesheetRow = DB_fetch_array($TimesheetResult)) {
					if ($TimesheetRow['status'] == 2) { //the timesheet is already posted - no changes are now possible
						...
						echo '<tr class="striped_row">
							...
							<td>', $TimesheetRow['workcentrename'], '</td>
							<td class="number"><input type="text" required="required" class="number" name="Day1_', $EditableRowNo, '" value="', locale_number_format($TimesheetRow['day1'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" /></td>
							<td class="number"><input type="text" required="required" class="number" name="Day2_', $EditableRowNo, '" value="', locale_number_format($TimesheetRow['day2'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" /></td>
							<td class="number"><input type="text" required="required" class="number" name="Day3_', $EditableRowNo, '" value="', locale_number_format($TimesheetRow['day3'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" /></td>
							<td class="number"><input type="text" required="required" class="number" name="Day4_', $EditableRowNo, '" value="', locale_number_format($TimesheetRow['day4'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" /></td>
							<td class="number"><input type="text" required="required" class="number" name="Day5_', $EditableRowNo, '" value="', locale_number_format($TimesheetRow['day5'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" /></td>
							<td class="number"><input type="text" required="required" class="number" name="Day6_', $EditableRowNo, '" value="', locale_number_format($TimesheetRow['day6'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" /></td>
							<td class="number"><input type="text" required="required" class="number" name="Day7_', $EditableRowNo, '" value="', locale_number_format($TimesheetRow['day7'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" /></td>
							<td class="number">', locale_number_format($TimesheetRow['day1'] + $TimesheetRow['day2'] + $TimesheetRow['day3'] + $TimesheetRow['day4'] + $TimesheetRow['day5'] + $TimesheetRow['day6'] + $TimesheetRow['day7'], $EmployeeRow['decimalplaces']), '</td>
							<td><a href="', htmlspecialchars($_SERVER['PHP_SELF'] . '?Delete=' . urlencode($TimesheetRow['id']) . '&SelectedEmployee=' . urlencode($SelectedEmployee) . '&WeekEnding=' . urlencode($_POST['WeekEnding']), ENT_QUOTES, 'UTF-8'), '"onclick="return confirm(\'', _('Are you sure you wish to delete this timesheet entry'), '\');">', _('Delete'), '</a></td>', (($TimesheetRow['status'] == '1') ? '<td>' . _('submitted') . '</td>' : ''), '</tr>';
			...
			echo '<td class="number">
				<input type="text" required="required" class="number" name="Day1" value="', locale_number_format($_POST['Day1'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" />
			</td>
			<td class="number">
				<input type="text" required="required" class="number" name="Day2" value="', locale_number_format($_POST['Day2'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" />
			</td>
			<td class="number">
				<input type="text" required="required" class="number" name="Day3" value="', locale_number_format($_POST['Day3'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" />
			</td>
			<td class="number">
				<input type="text" required="required" class="number" name="Day4" value="', locale_number_format($_POST['Day4'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" />
			</td>
			<td class="number">
				<input type="text" required="required" class="number" name="Day5" value="', locale_number_format($_POST['Day5'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" />
			</td>
			<td class="number">
				<input type="text" required="required" class="number" name="Day6" value="', locale_number_format($_POST['Day6'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" />
			</td>
			<td class="number">
				<input type="text" required="required" class="number" name="Day7" value="', locale_number_format($_POST['Day7'], $EmployeeRow['decimalplaces']), '" minlength="1" maxlength="4" size="4" />
			</td>
		</tr>';
		...
		if ($_SESSION['CanEnterTimesheets'] == 1 and isset($SelectedEmployee) and $EditableRowNo > 0) { //a timesheet administrator
			echo '<br />
			<hr />
