...
if (isset($_GET['SelectedGroup'])) {
	...
	if (isset($SelectedGroup)) {
		...
		if (DB_num_rows($UsedResult) == 0) {
			...
			echo '<div class="page_help_text">', _('As this is the first time that the system has been used, you must first create a tax province for this group.'), '<br />', _('For help, click on the help icon in the top right'), '<br />', _('Once you have filled in all the details, click on the button at the bottom of the screen'), '</div>';
			echo '<a href="', $RootPath, '/TaxProvinces.php">', _('Create a tax province'), '</a>';
			include ('includes/footer.phop');
			exit;
		}
